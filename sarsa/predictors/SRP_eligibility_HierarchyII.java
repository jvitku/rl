 ...
 *
 */
public class SRP_eligibility_HierarchyII implements StochasticReturnPredictor{
	...
	private final ComplexAction me;
	 ...
	 */
	public SRP_eligibility_HierarchyII(MyLogger log, 
			ArrayList<Action> childActions,			 
			ArrayList<Variable> childVars, 
			PhysiologicalStateSpace p, HarmSystemSettings set, ComplexAction me, 
			ActuatorHARM a){
		this.settings = set;
		this.me = me;
	...
	private double getEpsilon(){
		...
		Motivation m = this.me.getConnection().source();
	...
	private final double ee = 2.7;
	private final int coeff = 10000;
	/**
	 * generate action:
	...
	public void generateAction(){
		...
		double motivation = Math.pow(ee, this.me.getConnection().source().get())* coeff;
		
		int val = (int)(this.me.getPriority() + motivation +(this.QsaVal + this.addition));
		
		this.mypl("\t-------  \tee: "+ee+" \tsource "+
				this.me.getConnection().source().get()+" \tresult1: "+motivation+" " +
						"\toverall: "+val);
		/*
