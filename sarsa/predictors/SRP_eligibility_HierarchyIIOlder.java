 ...
 *
 */
public class SRP_eligibility_HierarchyIIOlder implements StochasticReturnPredictor{
	 ...
	 */
	public SRP_eligibility_HierarchyIIOlder(MyLogger log, 
	...
	public void infer(ActuatorHARM a){
		...
		if(prev.size() == this.Q.getDimension() && previousKnown){
			...
			this.updateKnowledgePrivate(re);
			//this.updateOld(re);
		}
	...
	private boolean getExecutedAction(Actuator a){
		...
		this.mypl("befre step: elig is: "+this.elig.eligibilitiesToString());
		elig.makeStepHere(prev);
		this.mypl("after step: elig is: "+this.elig.eligibilitiesToString());
		return true;
	}
	...
	/*
	private boolean getExecutedActionNew(Actuator a){
		this.mypl("action is this: "+a.getActionExecuted());
		...
		// action is not known to this system 
		if(executed == null){
			this.mypl("and it is unknwon , ignoring! ");
			return false;
		}
		//elig.makeStepHere(prev);
		return true;
	...
	*/
	...
	//	System.out.println("update: "+prev.size()+" "+Q.getDimension());
		...
		System.out.println("learning traces: ");
		elig.learn(delta, Q);
