 ...
 *
 */
public class IntentionalAction extends DecisionSpace implements ComplexAction{
	...
	public IntentionalAction(String name, HarmSystemSettings settings, MyLogger log, 
			PhysiologicalStateSpace s,
			ActuatorHARM ac, Variable myVar){
		
		super(false,name);
		this.myVar = myVar;
		this.reinforcedEnvVariable = this.myVar;
	...
	private void mypl(String what){ this.log.pl(LEVNEW,"IntentionalAction:"+this.name+": "+what); }
	...
	public int getMyReinforcementVal(){
		...
		if(!m.wasReinforced()) // TODO oooooooo
			return 0;
	...
	public void updateComplexity(){
		...
		for(int i=0; i<this.childActions.size(); i++){
			if((max < this.childActions.get(i).getComplexity()) && 
					(!this.IAmAlsoHisChild(this, this.childActions.get(i))))
	...
	private boolean IAmAlsoHisChild(Action father, Action child){
		ArrayList<Action> ch = child.getChilds();
		for(int i=0; i<ch.size(); i++){
			if(father.equals(ch.get(i)))
				return true;
		}
		return false;
	}
	...
	public int getReinforcerVar(){
		System.err.println("I am intentional! no reinforcedVAr by int..");
		return -1; 
	}

	public void setReinforcedStateSpaceVar(int var){ System.err.println("I am intentional"); } 
