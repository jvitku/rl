 ...
 *
 */
public class EligibilityQueueII {
	 ...
	 */
	public EligibilityQueueII(int length, double gamma, double lambda, double alpha){
		this.len = length;
		this.gamma = gamma;
		this.lambda = lambda;
		this.alpha = alpha;
	...
	public void learn(double delta, QSAMatrix q){
		...
		for(int i=0; i<inds.size(); i++){
			...
			/*
			System.out.println("size od infs is: "+this.inds.get(i).size()+" mat dim: "+
					q.getDimension());
			*/
			if(q.matrixChanged()){
				//System.out.println("matrix changed, discarding all traces   !!!!!!!");
				this.deleteAll();
				return;
			}
			//System.out.println("matrix not changed! learning traces!");
			// learning traces: 
		...
		//	System.out.println("getting.. "+i+" "+inds.get(i).size()+" "+q.getDimension());
			//System.out.println("dimension check here please................");
			actual = q.get(inds.get(i));
			//System.out.println("dimension check here please................end");
			
			if(actual == -1){	// not visited yet?
				//System.err.println("no co to tady jako tohle dela?!?!? :-)   xx");
				System.out.print(".");
				actual = 0;
			}
